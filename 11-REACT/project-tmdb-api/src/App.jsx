...
const MovieApp = () => {
  ...
  const fetchPopularMovies = async () => {
    try {
      const response = await fetch(
        `${BASE_URL}/movie/popular?api_key=${API_KEY}&language=en-EN&page=1`
      );
  ...
  const searchMovies = async () => {
    try {
      const response = await fetch(
        `${BASE_URL}/search/movie?api_key=${API_KEY}&language=en-EN&query=${searchTerm}`
      );
      const data = await response.json();
  ...
  const fetchMoviesByGenre = async () => {
    try {
      const response = await fetch(
        `${BASE_URL}/discover/movie?api_key=${API_KEY}&language=en-EN&with_genres=${selectedGenre}`
      );
      const data = await response.json();
      setMovies(data.results);
    } catch (error) {
        ...
        <div className="mb-8">
          ...
          <div className="flex gap-4 flex-wrap">
            ...
            >
              ...
              {
                searchTerm.length === 0 && selectedGenre == '' ? (
                  <h2 className="text-4xl text-center py-4 font-bold text-gray-900 mb-4">TOP 10 MOVIES</h2>
                ) : (
                  <h2 className="text-4xl text-center py-4 font-bold text-gray-900 mb-4">{selectedGenre ? genres.find((genre) => genre.id === parseInt(selectedGenre))?.name : 'Paieškos rezultatai'}</h2>
